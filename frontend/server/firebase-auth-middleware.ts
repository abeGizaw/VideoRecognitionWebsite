...
export const firebaseAuthMiddleware: Get<[], UniversalMiddleware> =
...
export const firebaseAuthLoginHandler: Get<[], UniversalHandler> =
  () => async (request) => {
...
export const firebaseAuthLogoutHandler: Get<[], UniversalHandler> =
  () => async () => {
    return new Response(JSON.stringify({ status: 'success' }), {
      status: 200,
      headers: {
